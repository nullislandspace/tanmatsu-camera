...
static volatile uint8_t *s_cam_stable =
    NULL; // task-side: most-recently-completed
static volatile bool s_photo_lock =
    false; // when set, ISR stops toggling stable/active

static uint8_t *s_preview_buffer =
    NULL; // Scaled + rotated preview RGB565 (panel-native)
static size_t s_preview_buffer_sz = 0;
static uint32_t s_preview_w =
    0; // buffer width  (stride in pixels, panel-native)
...
static SemaphoreHandle_t s_transfer_done =
    NULL;                                   // given from CSI trans_finished ISR
static SemaphoreHandle_t s_srm_done = NULL; // given from PPA srm_done ISR
static SemaphoreHandle_t s_frame_ready =
    NULL; // given to the main loop after PPA
static SemaphoreHandle_t s_render_ready =
    NULL; // given by the main loop after it has
          // finished drawing; gates the next PPA
          // transform so it doesn't overwrite the
          // preview buffer mid-draw.
static SemaphoreHandle_t s_ppa_mutex =
    NULL; // serialises s_ppa access between the
...
static bool IRAM_ATTR on_srm_done(ppa_client_handle_t ppa_handle,
                                  ppa_event_data_t *event, void *user_data) {
  s_cnt_srm_done++;
...
static void render_task(void *arg) {
  ...
  while (s_running) {
    ...
    if (xSemaphoreTake(s_transfer_done, pdMS_TO_TICKS(100)) != pdTRUE) {
      ...
      if ((n_timeouts % 20) == 0) {
        ESP_LOGW(TAG,
                 "no transfer_done: %" PRIu32 " timeouts, frames=%" PRIu32
                 "  ISR cnts get=%" PRIu32 " fin=%" PRIu32 " srm=%" PRIu32,
                 n_timeouts, n_frames, s_cnt_get_new_trans,
                 s_cnt_trans_finished, s_cnt_srm_done);
      }
      continue;
    }
    if (!s_running)
    ...
    if (n_frames <= 3 || (n_frames % 30) == 0) {
        ESP_LOGI(TAG, "frame %" PRIu32 "  ISR get=%" PRIu32 " fin=%" PRIu32
                 ...
                 n_frames, s_cnt_get_new_trans, s_cnt_trans_finished,
    s_cnt_srm_done, (long long)(t_mutex_out - t_mutex_in), (long
    long)(t_ppa_done - t_mutex_out));
    }
    */
  }
...
esp_err_t camera_preview_start(const camera_source_t *src, uint32_t req_w,
  ...
  ESP_LOGI(TAG,
           "preview buf %" PRIu32 "x%" PRIu32 " (panel-native, scale %" PRIu32
           "/16) cache_line=%" PRIu32,
  ...
  for (int i = 0; i < 2; i++) {
    s_cam_buf[i] = heap_caps_aligned_calloc(
        cache_line, 1, s_cam_buf_sz, MALLOC_CAP_8BIT | MALLOC_CAP_SPIRAM);
    if (!s_cam_buf[i]) {
  ...
  s_cam_active_idx = 0;
  s_cam_stable =
      NULL; // nothing stable yet; render_task bails until first frame
  s_photo_lock = false;
  ...
  s_preview_buffer_sz =
      (s_preview_buffer_sz + cache_line - 1u) & ~((size_t)cache_line - 1u);
  s_preview_buffer = heap_caps_aligned_calloc(
      cache_line, 1, s_preview_buffer_sz, MALLOC_CAP_SPIRAM | MALLOC_CAP_DMA);
  if (!s_preview_buffer) {
...
void camera_preview_stop(void) {
  ...
  if (s_render_ready) {
    vSemaphoreDelete(s_render_ready);
    s_render_ready = NULL;
  }
  if (s_ppa_mutex) {
    vSemaphoreDelete(s_ppa_mutex);
    s_ppa_mutex = NULL;
  }
...
const uint8_t *camera_preview_get_raw_pixels(void) {
  return (const uint8_t *)s_cam_stable;
}
...
esp_err_t camera_video_snapshot(uint8_t *out_buf, size_t out_buf_sz,
  ...
  if (out_buf == NULL || out_w == 0 || out_h == 0 || stride_w < out_w ||
      stride_h < out_h) {
    return ESP_ERR_INVALID_ARG;
  }
  if (!s_running || s_ppa == NULL) {
  ...
  if (n_from_w == 0 || n_from_w > 16)
    return ESP_ERR_INVALID_ARG;
  if ((s_src_w * n_from_w) / 16u != out_w)
    return ESP_ERR_INVALID_ARG;
  if ((s_src_h * n_from_w) / 16u != out_h)
    return ESP_ERR_INVALID_ARG;
