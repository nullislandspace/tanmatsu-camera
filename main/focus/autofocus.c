...
on_stats_done(isp_af_ctlr_t af, const esp_isp_af_env_detector_evt_data_t *edata,
              void *user_data) {
...
esp_err_t autofocus_init(isp_proc_handle_t isp, uint16_t input_w,
  ...
  s_last_def = 0;
  ESP_LOGI(TAG, "ready, window=(%u,%u)..(%u,%u), enabled=%d", (unsigned)x,
           (unsigned)y, (unsigned)(x + w - 1), (unsigned)(y + h - 1),
           (int)s_enabled);
  return ESP_OK;
...
af_hud_state_t autofocus_hud_state(void) {
  if (!s_enabled)
    return AF_HUD_OFF;
  if (s_override)
    return AF_HUD_OVERRIDE;
  if (s_state == AF_LOCKED)
    return AF_HUD_LOCK;
  if (s_state == AF_IDLE)
    return AF_HUD_OFF;
...
int autofocus_hud_sharpness(void) { return s_last_def; }
...
void autofocus_tick(uint16_t *focus_pos) {
  ...
  if (s_state == AF_OVERRIDE) {
    ...
    s_fine_origin =
        (*focus_pos > AF_FINE_STEP * (AF_FINE_SAMPLES / 2))
  ...
  case AF_COARSE: {
    ...
    if (np > fd->pos_max) {
      ...
      int32_t origin =
          (int32_t)s_best_pos - AF_FINE_STEP * (AF_FINE_SAMPLES / 2);
      if (origin < fd->pos_min)
        origin = fd->pos_min;
  ...
  case AF_FINE: {
    ...
    if (np > end || np > fd->pos_max) {
      ...
      s_lock_age_frames = 0;
      ESP_LOGI(TAG, "locked at pos=%u def=%d", (unsigned)s_best_pos,
               s_best_def);
    } else {
      next = (uint16_t)np;
      move = true;
  ...
  case AF_LOCKED:
    ...
    if (s_lock_age_frames >= AF_LOCK_RECHECK) {
      ...
      if (def < threshold) {
        ESP_LOGI(TAG, "drift: def=%d < %d (%d%% of %d) → re-acquire", def,
                 threshold, AF_LOCK_DROP_PCT, s_locked_def);
        s_state = AF_COARSE;
        s_best_def = 0;
        s_best_pos = cur;
        s_settle_frames = 0;
