...
esp_err_t dw9714p_init(void) {
  ...
  esp_err_t err = ensure_dev_locked();
  if (err == ESP_OK)
    err = write_word_locked(DW9714P_CMD_PROTECT_OFF);
  if (err == ESP_OK)
    err = write_word_locked(DW9714P_CMD_DLC_MCLK);
  if (err == ESP_OK)
    err = write_word_locked(DW9714P_CMD_TSRC);
  if (err == ESP_OK)
    err = write_word_locked(DW9714P_CMD_PROTECT_ON);
  bsp_i2c_primary_bus_release();
