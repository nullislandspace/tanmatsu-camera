...
void splash_show(fbdraw_t *fb) {
  ...
  if (!f)
    f = fastopen(SPLASH_PATH_INT, "rb");
  if (!f) {
    ESP_LOGW(TAG, "splash not found at %s or %s", SPLASH_PATH_SD,
             SPLASH_PATH_INT);
    return;
  }
  ...
  err = jpeg_decoder_process(dec, &decode_cfg, in_buf, (uint32_t)fsz, out_buf,
                             out_alloc, &dec_out_sz);
  free(in_buf);
  jpeg_del_decoder_engine(dec);
  if (err != ESP_OK) {
  ...
  if (dy < 0)
    dy = 0;
  fbdraw_blit_rotated(fb, dx, dy, (const uint16_t *)out_buf, info.width,
                      info.height);
  free(out_buf);
