...
esp_err_t bmp_writer_save_rgb565(const char *path, const uint16_t *pixels,
  ...
  const uint32_t headers_size =
      sizeof(bmp_file_header_t) + sizeof(bmp_info_header_t);
  ...
  uint8_t *row_buf = heap_caps_malloc(
      padded_row, MALLOC_CAP_8BIT | MALLOC_CAP_DMA | MALLOC_CAP_INTERNAL);
  if (!row_buf) {
  ...
  fastclose(f);
  free(row_buf);
  ESP_LOGI(TAG, "wrote %s (%" PRIu32 "x%" PRIu32 ", %" PRIu32 " bytes)", path,
           width, height, (uint32_t)(headers_size + image_bytes));
  return ESP_OK;
}
