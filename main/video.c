...
typedef struct {
  ...
  uint32_t video_frame_ix; // sequential frame number (for keyframe detection
                           // via GOP)
} video_rec_t;
...
static void fill_audio_frame(int16_t *dst, int samples) {
  ...
  if (win_start_us == 0)
    win_start_us = esp_timer_get_time();
  total_got += got;
...
static void writer_task_fn(void *arg) {
  ...
  while (1) {
    ...
    if (pick_v) {
      ...
      esp_err_t werr =
          avi_mux_write_video(&s_rec.mux, item.data, item.size, item.keyframe);
      if (werr != ESP_OK)
        ESP_LOGE(TAG, "writer: avi video: %d", werr);
    } else {
      xQueueReceive(s_rec.audio_q, &item, 0);
      esp_err_t werr =
          avi_mux_write_audio(&s_rec.mux, item.data, item.size, item.samples);
      if (werr != ESP_OK)
        ESP_LOGE(TAG, "writer: avi audio: %d", werr);
    }
    free(item.data);
  }
...
static void video_task_fn(void *arg) {
  ...
  while (s_rec.running) {
    ...
    esp_err_t err =
        camera_video_snapshot(s_rec.yuv_buf, YUV_BUF_SIZE, VIDEO_W, VIDEO_H,
    ...
    if (!payload) {
      ESP_LOGE(TAG, "video chunk malloc failed (%" PRIu32 " bytes)",
               out.length);
      continue;
    }
    ...
    if (acc_n >= 15) {
      ESP_LOGI(
          TAG,
          "v: snap=%llu enc=%llu write=%llu  avg us/frame (n=%d, len=%" PRIu32
          ")",
          (unsigned long long)(acc_snap_us / acc_n),
          (unsigned long long)(acc_enc_us / acc_n),
          (unsigned long long)(acc_write_us / acc_n), acc_n, out.length);
      acc_snap_us = acc_enc_us = acc_write_us = 0;
      acc_n = 0;
    }
  }

  ESP_LOGI(TAG, "video task exit (queued %" PRIu32 " frames)",
           s_rec.video_frame_ix);
  ...
  while (s_rec.running) {
    ...
    if (mp3 != NULL && written > 0) {
      ...
      if (!first_header_logged && written >= 4) {
        ESP_LOGI(TAG, "mp3 hdr: %02x %02x %02x %02x  len=%d", mp3[0], mp3[1],
                 mp3[2], mp3[3], written);
    ...
    if (acc_n >= 25) {
      ESP_LOGI(TAG,
               "a: gen=%llu enc=%llu write=%llu  avg us/frame (n=%d, len=%d)",
               (unsigned long long)(acc_gen_us / acc_n),
               (unsigned long long)(acc_enc_us / acc_n),
               (unsigned long long)(acc_write_us / acc_n), acc_n, written);
...
static esp_err_t setup_h264_encoder(void) {
  esp_h264_enc_cfg_hw_t cfg = {
      .pic_type = ESP_H264_RAW_FMT_O_UYY_E_VYY, // universal — works on all P4
                                                // silicon revs
...
static esp_err_t setup_shine(void) {
  ...
  s_rec.pcm_scratch =
      heap_caps_calloc(s_rec.shine_samples_per_frame, sizeof(int16_t),
...
static void teardown_all(void) {
  ...
  if (s_rec.pcm_scratch) {
    free(s_rec.pcm_scratch);
    s_rec.pcm_scratch = NULL;
  }
...
esp_err_t video_record_start(const char *dcim_dir, char *out_path,
  ...
  err = avi_mux_open(&s_rec.mux, s_rec.path, VIDEO_W, VIDEO_H, VIDEO_FPS,
  ...
  BaseType_t ok1 =
      xTaskCreate(video_task_fn, "vid_rec", 6144, NULL, 6, &s_rec.video_task);
  BaseType_t ok2 = xTaskCreatePinnedToCore(audio_task_fn, "aud_rec", 6144, NULL,
                                           6, &s_rec.audio_task, 1);
  BaseType_t ok3 =
      xTaskCreate(writer_task_fn, "avi_wr", 4096, NULL, 4, &s_rec.writer_task);
...
esp_err_t video_record_stop(void) {
  ...
  s_rec.running = false;
  for (int i = 0; i < 100; ++i) {
    if (!s_rec.video_task && !s_rec.audio_task)
      break;
...
bool video_is_recording(void) { return s_rec.running; }
