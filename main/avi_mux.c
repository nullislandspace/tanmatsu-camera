...
static int wr_fcc(FILE *f, uint32_t fcc) { return wr_u32(f, fcc); }
...
static esp_err_t idx_reserve(avi_mux_t *mux, uint32_t want_count) {
  if (want_count <= mux->idx_capacity)
    return ESP_OK;
  uint32_t new_cap = mux->idx_capacity ? mux->idx_capacity * 2u : 512u;
  if (new_cap < want_count)
    new_cap = want_count + 256u;
  ...
  if (!p) {
    ...
    p = heap_caps_realloc(mux->idx_entries, new_bytes,
...
static esp_err_t idx_append(avi_mux_t *mux, uint32_t fourcc, uint32_t flags,
  ...
  slot[0] = (uint8_t)fourcc;
  slot[1] = (uint8_t)(fourcc >> 8);
  slot[2] = (uint8_t)(fourcc >> 16);
  slot[3] = (uint8_t)(fourcc >> 24);
  slot[4] = (uint8_t)flags;
  slot[5] = (uint8_t)(flags >> 8);
  slot[6] = (uint8_t)(flags >> 16);
  slot[7] = (uint8_t)(flags >> 24);
  slot[8] = (uint8_t)rel_offset;
  slot[9] = (uint8_t)(rel_offset >> 8);
  slot[10] = (uint8_t)(rel_offset >> 16);
  slot[11] = (uint8_t)(rel_offset >> 24);
  slot[12] = (uint8_t)size;
  slot[13] = (uint8_t)(size >> 8);
  slot[14] = (uint8_t)(size >> 16);
  slot[15] = (uint8_t)(size >> 24);
  mux->idx_count++;
  return ESP_OK;
}
...
static esp_err_t write_headers(avi_mux_t *mux) {
  ...
  // RIFF <size> AVI
  if (wr_fcc(f, FCC_RIFF))
    return ESP_FAIL;
  mux->riff_size_pos = cur_pos(f);
  if (wr_u32(f, 0))
    return ESP_FAIL; // patched at close
  if (wr_fcc(f, FCC_AVI))
    return ESP_FAIL;

  // LIST <size> hdrl
  if (wr_fcc(f, FCC_LIST))
    return ESP_FAIL;
  size_t hdrl_size_pos = cur_pos(f);
  if (wr_u32(f, 0))
    return ESP_FAIL; // patched below once we know the size
  size_t hdrl_data_start = cur_pos(f);
  if (wr_fcc(f, FCC_HDRL))
    return ESP_FAIL;

  // avih <56> <MainAVIHeader>
  if (wr_fcc(f, FCC_AVIH))
    return ESP_FAIL;
  if (wr_u32(f, 56))
    return ESP_FAIL;
  uint32_t us_per_frame = mux->video_fps ? 1000000u / mux->video_fps : 66666u;
  if (wr_u32(f, us_per_frame))
    return ESP_FAIL; // dwMicroSecPerFrame
  mux->avih_max_bytes_per_sec_pos = cur_pos(f);
  if (wr_u32(f, 0))
    return ESP_FAIL; // dwMaxBytesPerSec (patched)
  if (wr_u32(f, 0))
    return ESP_FAIL; // dwPaddingGranularity
  if (wr_u32(f, 0x10 | 0x20))
    return ESP_FAIL; // dwFlags (AVIF_HASINDEX | AVIF_ISINTERLEAVED)
  mux->avih_total_frames_pos = cur_pos(f);
  if (wr_u32(f, 0))
    return ESP_FAIL; // dwTotalFrames (patched)
  if (wr_u32(f, 0))
    return ESP_FAIL; // dwInitialFrames
  if (wr_u32(f, 2))
    return ESP_FAIL; // dwStreams
  if (wr_u32(f, 0x10000))
    return ESP_FAIL; // dwSuggestedBufferSize (~64 KB)
  if (wr_u32(f, mux->video_width))
    return ESP_FAIL; // dwWidth
  if (wr_u32(f, mux->video_height))
    return ESP_FAIL; // dwHeight
  for (int i = 0; i < 4; i++)
    if (wr_u32(f, 0))
      return ESP_FAIL; // dwReserved[4]

  // LIST <size> strl (video stream)
  if (wr_fcc(f, FCC_LIST))
    return ESP_FAIL;
  if (wr_u32(f, 4 + 8 + 56 + 8 + 40))
    return ESP_FAIL; // LIST payload = "strl" + strh chunk + strf chunk
  if (wr_fcc(f, FCC_STRL))
    return ESP_FAIL;

  // strh video (56 bytes)
  if (wr_fcc(f, FCC_STRH))
    return ESP_FAIL;
  if (wr_u32(f, 56))
    return ESP_FAIL;
  if (wr_fcc(f, FCC_VIDS))
    return ESP_FAIL; // fccType
  if (wr_fcc(f, FCC_H264))
    return ESP_FAIL; // fccHandler
  if (wr_u32(f, 0))
    return ESP_FAIL; // dwFlags
  if (wr_u16(f, 0))
    return ESP_FAIL; // wPriority
  if (wr_u16(f, 0))
    return ESP_FAIL; // wLanguage
  if (wr_u32(f, 0))
    return ESP_FAIL; // dwInitialFrames
  if (wr_u32(f, 1))
    return ESP_FAIL; // dwScale
  if (wr_u32(f, mux->video_fps))
    return ESP_FAIL; // dwRate
  if (wr_u32(f, 0))
    return ESP_FAIL; // dwStart
  mux->vids_length_pos = cur_pos(f);
  if (wr_u32(f, 0))
    return ESP_FAIL; // dwLength (patched)
  if (wr_u32(f, 0x10000))
    return ESP_FAIL; // dwSuggestedBufferSize
  if (wr_u32(f, 0xFFFFFFFFu))
    return ESP_FAIL; // dwQuality (-1 = unspecified)
  if (wr_u32(f, 0))
    return ESP_FAIL; // dwSampleSize (video: 0 = VBR)
  // rcFrame left=0, top=0, right=width, bottom=height
  if (wr_u16(f, 0))
    return ESP_FAIL;
  if (wr_u16(f, 0))
    return ESP_FAIL;
  if (wr_u16(f, (uint16_t)mux->video_width))
    return ESP_FAIL;
  if (wr_u16(f, (uint16_t)mux->video_height))
    return ESP_FAIL;

  // strf video: BITMAPINFOHEADER (40 bytes)
  if (wr_fcc(f, FCC_STRF))
    return ESP_FAIL;
  if (wr_u32(f, 40))
    return ESP_FAIL;
  if (wr_u32(f, 40))
    return ESP_FAIL; // biSize
  if (wr_u32(f, mux->video_width))
    return ESP_FAIL; // biWidth
  if (wr_u32(f, mux->video_height))
    return ESP_FAIL; // biHeight
  if (wr_u16(f, 1))
    return ESP_FAIL; // biPlanes
  if (wr_u16(f, 24))
    return ESP_FAIL; // biBitCount
  if (wr_fcc(f, FCC_H264))
    return ESP_FAIL; // biCompression
  if (wr_u32(f, mux->video_width * mux->video_height * 3u))
    return ESP_FAIL; // biSizeImage
  if (wr_u32(f, 0))
    return ESP_FAIL; // biXPelsPerMeter
  if (wr_u32(f, 0))
    return ESP_FAIL; // biYPelsPerMeter
  if (wr_u32(f, 0))
    return ESP_FAIL; // biClrUsed
  if (wr_u32(f, 0))
    return ESP_FAIL; // biClrImportant

  // LIST <size> strl (audio stream)
  if (wr_fcc(f, FCC_LIST))
    return ESP_FAIL;
  // LIST payload = "strl" + strh chunk + strf chunk (strf payload 30 bytes for
  // MPEGLAYER3)
  if (wr_u32(f, 4 + 8 + 56 + 8 + 30))
    return ESP_FAIL;
  if (wr_fcc(f, FCC_STRL))
    return ESP_FAIL;
  ...
  if (wr_u32(f, 56))
    return ESP_FAIL;
  if (wr_fcc(f, FCC_AUDS))
    return ESP_FAIL; // fccType
  if (wr_fcc(f, 0))
    return ESP_FAIL; // fccHandler (0 for audio by convention)
  if (wr_u32(f, 0))
    return ESP_FAIL; // dwFlags
  if (wr_u16(f, 0))
    return ESP_FAIL; // wPriority
  ...
  if (wr_u32(f, 0))
    return ESP_FAIL; // dwInitialFrames
  if (wr_u32(f, mux->audio_samples_per_frame))
    return ESP_FAIL; // dwScale = samples/frame
  if (wr_u32(f, mux->audio_sample_rate))
    return ESP_FAIL; // dwRate = sample rate
  if (wr_u32(f, 0))
    return ESP_FAIL; // dwStart
  mux->auds_length_pos = cur_pos(f);
  if (wr_u32(f, 0))
    return ESP_FAIL; // dwLength (patched: total MP3 frames at close)
  if (wr_u32(f, 0x4000))
    return ESP_FAIL; // dwSuggestedBufferSize (~16 KB)
  if (wr_u32(f, 0xFFFFFFFFu))
    return ESP_FAIL; // dwQuality
  if (wr_u32(f, 0))
    return ESP_FAIL; // dwSampleSize = 0 (VBR/framed)
  if (wr_u16(f, 0))
    return ESP_FAIL;
  if (wr_u16(f, 0))
    return ESP_FAIL;
  ...
  // strf audio: MPEGLAYER3WAVEFORMAT (30 bytes = WAVEFORMATEX(18) +
  // MPEGLAYER3(12)) Required for Windows compatibility and matches what ffmpeg
  // writes.
  if (wr_fcc(f, FCC_STRF))
    return ESP_FAIL;
  if (wr_u32(f, 30))
    return ESP_FAIL;
  if (wr_u16(f, 0x0055))
    return ESP_FAIL; // wFormatTag: WAVE_FORMAT_MPEGLAYER3
  if (wr_u16(f, mux->audio_channels))
    return ESP_FAIL; // nChannels
  if (wr_u32(f, mux->audio_sample_rate))
    return ESP_FAIL; // nSamplesPerSec
  if (wr_u32(f, mux->audio_avg_bytes_per_sec))
    return ESP_FAIL; // nAvgBytesPerSec
  if (wr_u16(f, mux->audio_samples_per_frame))
    return ESP_FAIL; // nBlockAlign = samples/frame (matches ffmpeg)
  if (wr_u16(f, 0))
    return ESP_FAIL; // wBitsPerSample (0 for MP3)
  if (wr_u16(f, 12))
    return ESP_FAIL; // cbSize (12 = MPEGLAYER3 extension)
  // MPEGLAYER3WAVEFORMAT extension
  if (wr_u16(f, 1))
    return ESP_FAIL; // wID = MPEGLAYER3_ID_MPEG
  if (wr_u32(f, 2))
    return ESP_FAIL; // fdwFlags = MPEGLAYER3_FLAG_PADDING_OFF
  if (wr_u16(f, mux->audio_samples_per_frame))
    return ESP_FAIL; // nBlockSize (samples per MPEG frame: 1152 Layer-III
                     // MPEG-I, 576 MPEG-II)
  if (wr_u16(f, 1))
    return ESP_FAIL; // nFramesPerBlock
  if (wr_u16(f, 1393))
    return ESP_FAIL; // nCodecDelay (Fraunhofer default for MP3)
  ...
  if (fseek(f, (long)hdrl_size_pos, SEEK_SET))
    return ESP_FAIL;
  if (wr_u32(f, hdrl_list_size))
    return ESP_FAIL;
  if (fseek(f, (long)hdrl_end, SEEK_SET))
    return ESP_FAIL;
  ...
  if (wr_u32(f, 0))
    return ESP_FAIL;                 // movi LIST size (patched at close)
  mux->movi_data_start = cur_pos(f); // points at the "movi" FourCC
  if (wr_fcc(f, FCC_MOVI))
    return ESP_FAIL;

  return ESP_OK;
}

esp_err_t avi_mux_open(avi_mux_t *mux, const char *path, uint32_t video_width,
  ...
  mux->is_open = true;
  ESP_LOGI(TAG,
           "opened %s  video %" PRIu32 "x%" PRIu32 " @%" PRIu32
           " fps  audio %" PRIu32 " Hz x%u",
           path, video_width, video_height, video_fps, audio_sample_rate,
...
static esp_err_t write_chunk(avi_mux_t *mux, uint32_t fourcc, const void *data,
                             size_t size, size_t *out_data_offset) {
  FILE *f = mux->f;
  if (wr_fcc(f, fourcc))
    return ESP_FAIL;
  if (wr_u32(f, (uint32_t)size))
    return ESP_FAIL;
  size_t data_off = cur_pos(f);
  if (size > 0) {
    if (wr_bytes(f, data, size))
      return ESP_FAIL;
  }
  if (size & 1u) {
    uint8_t pad = 0;
...
esp_err_t avi_mux_write_video(avi_mux_t *mux, const void *data, size_t size,
                              bool keyframe) {
  if (!mux || !mux->is_open)
  ...
  err = idx_append(mux, FCC_00DC, keyframe ? AVIIF_KEYFRAME : 0u, rel,
                   (uint32_t)size);
  if (err != ESP_OK)
    return err;
...
esp_err_t avi_mux_close(avi_mux_t *mux) {
  ...
  if (wr_fcc(f, FCC_IDX1))
    return ESP_FAIL;
  uint32_t idx_bytes = mux->idx_count * 16u;
  if (wr_u32(f, idx_bytes))
    return ESP_FAIL;
  if (idx_bytes > 0) {
    if (wr_bytes(f, mux->idx_entries, idx_bytes))
      return ESP_FAIL;
  }
  ...
  uint32_t movi_list_size =
      (uint32_t)(movi_end - mux->movi_data_start) +
      4u; // +4 because LIST size includes the "movi" FourCC
  ...
  if (fseek(f, (long)mux->movi_list_start, SEEK_SET))
    return ESP_FAIL;
  if (wr_u32(f, movi_list_size))
    return ESP_FAIL;
  ...
  if (fseek(f, (long)mux->riff_size_pos, SEEK_SET))
    return ESP_FAIL;
  if (wr_u32(f, riff_size))
    return ESP_FAIL;

  // Patch avih.dwTotalFrames and dwMaxBytesPerSec.
  if (fseek(f, (long)mux->avih_total_frames_pos, SEEK_SET))
    return ESP_FAIL;
  if (wr_u32(f, mux->video_frames_written))
    return ESP_FAIL;
  // Rough max bytes/sec estimate: video_bytes + audio_bytes divided
  // by duration in seconds. Guard against zero fps / zero frames.
  uint32_t dur_s =
      mux->video_fps ? (mux->video_frames_written / mux->video_fps) : 0u;
  uint32_t total_bps = 0;
  if (dur_s > 0) {
    total_bps = (mux->video_bytes_written + mux->audio_bytes_written) / dur_s;
  }
  if (fseek(f, (long)mux->avih_max_bytes_per_sec_pos, SEEK_SET))
    return ESP_FAIL;
  if (wr_u32(f, total_bps))
    return ESP_FAIL;
  ...
  if (fseek(f, (long)mux->vids_length_pos, SEEK_SET))
    return ESP_FAIL;
  if (wr_u32(f, mux->video_frames_written))
    return ESP_FAIL;
  if (fseek(f, (long)mux->auds_length_pos, SEEK_SET))
    return ESP_FAIL;
  if (wr_u32(f, mux->audio_frames_written))
    return ESP_FAIL;
  ...
  ESP_LOGI(TAG,
           "closed: %" PRIu32 " video frames, %" PRIu32
