...
esp_err_t fbdraw_init(fbdraw_t *fb, size_t panel_w, size_t panel_h) {
  if (fb == NULL || panel_w == 0 || panel_h == 0)
...
void fbdraw_put_pixel(fbdraw_t *fb, int user_x, int user_y, uint16_t color) {
  ...
  if (user_x < 0 || user_y < 0)
    return;
  if ((size_t)user_x >= fb->user_w || (size_t)user_y >= fb->user_h)
    return;
...
void fbdraw_fill_rect(fbdraw_t *fb, int user_x, int user_y, int user_w,
  ...
  if (y0 < 0)
    y0 = 0;
  if ((size_t)x1 > fb->user_w)
    x1 = (int)fb->user_w;
  if ((size_t)y1 > fb->user_h)
    y1 = (int)fb->user_h;
  if (x0 >= x1 || y0 >= y1)
    return;
...
void fbdraw_blit_panel(fbdraw_t *fb, int panel_x, int panel_y,
                       ...
                       size_t src_panel_h) {
  if (fb == NULL || fb->pixels == NULL || src == NULL)
    return;
  if (panel_x < 0 || panel_y < 0)
    return;
  if ((size_t)panel_x + src_panel_w > fb->panel_w)
    return;
  if ((size_t)panel_y + src_panel_h > fb->panel_h)
    return;

  const size_t row_bytes = src_panel_w * sizeof(uint16_t);
  for (size_t row = 0; row < src_panel_h; row++) {
    uint16_t *dst =
        fb->pixels + ((size_t)panel_y + row) * fb->panel_w + (size_t)panel_x;
...
void fbdraw_blit_rotated(fbdraw_t *fb, int dst_x, int dst_y,
  ...
  if (dst_x < 0 || dst_y < 0)
    return;
  if ((size_t)dst_x + src_w > fb->user_w)
    return;
  if ((size_t)dst_y + src_h > fb->user_h)
    return;
...
int fbdraw_hershey_char(fbdraw_t *fb, uint16_t color, int sx, int sy, char c,
                        int font_height) {
