...
static camera_sensor_kind_t name_to_kind(const char *name) {
  ...
  if (strcmp(name, "OV5647") == 0)
    return CAMERA_SENSOR_OV5647;
  if (strcmp(name, "OV5640") == 0)
    return CAMERA_SENSOR_OV5640;
  if (strcmp(name, "OV5645") == 0)
...
static void capture_fps_base(camera_sensor_t *sensor,
                             const esp_cam_sensor_format_t *fmt) {
  if (sensor == NULL || fmt == NULL)
    return;
  ...
  if (camera_sensor_read_reg(sensor, OV_REG_TIMING_VTS_H, &vts_h) != ESP_OK)
    return;
  if (camera_sensor_read_reg(sensor, OV_REG_TIMING_VTS_L, &vts_l) != ESP_OK)
    return;
...
const char *camera_sensor_name(const camera_sensor_t *sensor) {
  if (sensor == NULL || sensor->device == NULL ||
      sensor->device->name == NULL) {
    return "?";
  }
...
esp_err_t camera_sensor_set_format_photo(camera_sensor_t *sensor,
  ...
  for (uint32_t i = 0; i < fmt_array.count; ++i) {
    ...
    if (f->port != ESP_CAM_SENSOR_MIPI_CSI)
      continue;
    if (best == NULL ||
        (uint64_t)f->width * f->height > (uint64_t)best->width * best->height) {
      best = f;
    }
  }
  ...
  ESP_LOGI(TAG, "Photo format: %s (%" PRIu32 "x%" PRIu32 ")", best->name,
...
esp_err_t camera_sensor_stream(camera_sensor_t *sensor, bool enable) {
  ...
  bsp_i2c_primary_bus_claim();
  esp_err_t err =
      esp_cam_sensor_ioctl(sensor->device, ESP_CAM_SENSOR_IOC_S_STREAM, &flag);
  bsp_i2c_primary_bus_release();
  if (err != ESP_OK) {
...
esp_err_t camera_sensor_read_reg(camera_sensor_t *sensor, uint16_t regaddr,
                                 uint8_t *out_val) {
  if (sensor == NULL || sensor->device == NULL || out_val == NULL)
    return ESP_ERR_INVALID_ARG;
  esp_cam_sensor_reg_val_t rv = {.regaddr = regaddr, .value = 0};
  bsp_i2c_primary_bus_claim();
  esp_err_t err =
      esp_cam_sensor_ioctl(sensor->device, ESP_CAM_SENSOR_IOC_G_REG, &rv);
  bsp_i2c_primary_bus_release();
  if (err != ESP_OK) {
...
esp_err_t camera_sensor_write_reg(camera_sensor_t *sensor, uint16_t regaddr,
  ...
  bsp_i2c_primary_bus_claim();
  esp_err_t err =
      esp_cam_sensor_ioctl(sensor->device, ESP_CAM_SENSOR_IOC_S_REG, &rv);
  bsp_i2c_primary_bus_release();
  if (err != ESP_OK) {
...
esp_err_t camera_sensor_set_preview_fps(camera_sensor_t *sensor,
  ...
  uint32_t vts = (sensor->base_vts_lines * sensor->base_fps + target_fps / 2u) /
                 target_fps;
  if (vts > 0xFFFFu)
    vts = 0xFFFFu;

  esp_err_t err = camera_sensor_write_reg(sensor, OV_REG_TIMING_VTS_H,
                                          (uint8_t)((vts >> 8) & 0xFFu));
  if (err != ESP_OK)
    return err;
  err = camera_sensor_write_reg(sensor, OV_REG_TIMING_VTS_L,
                                (uint8_t)(vts & 0xFFu));
  if (err != ESP_OK)
    return err;

  ESP_LOGI(TAG,
           "preview fps override: %" PRIu32 " fps (VTS=%" PRIu32
           ", base=%" PRIu32 "@%" PRIu32 "fps)",
