...
/*
 * fastopen() uses fast IO buffers for buffered STDIO. In tests, this seems to
 * sometimes increase read/write speeds up to 10x
 */
...
FILE *fastopen(const char *path, const char *mode) {
  ...
  if (path_needs_fast_io(path)) {
    void *buf = heap_caps_malloc(CONFIG_FATFS_STDIO_BUF_SIZE,
...
void fastclose(FILE *f) {
  ...
  // Not tracked (either not a fast path or buffer allocation failed), just
  // close
  fclose(f);
}
...
FILE *fastopen(const char *path, const char *mode) { return fopen(path, mode); }
