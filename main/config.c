...
esp_err_t config_save(const camera_config_t *cfg) {
    ...
    fclose(f);
    ESP_LOGI(TAG, "saved %s (focus_enabled=%d)", CONFIG_PATH, cfg->focus_enabled);
    return ESP_OK;
}
...
esp_err_t config_load(camera_config_t *out) {
    ...
    fclose(f);
    ESP_LOGI(TAG, "loaded %s (focus_enabled=%d)",
             CONFIG_PATH, out->focus_enabled);
    return ESP_OK;
}
