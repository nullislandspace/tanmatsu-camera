...
static int clamp_mic_gain(int v) {
  if (v < CONFIG_MIC_GAIN_MIN)
    return CONFIG_MIC_GAIN_MIN;
  if (v > CONFIG_MIC_GAIN_MAX)
    return CONFIG_MIC_GAIN_MAX;
  return v;
}
...
esp_err_t config_save(const camera_config_t *cfg) {
  ...
  ESP_LOGI(TAG, "saved %s (driver=%s focus=%d af=%d rot180=%d mic=%d gain=%d)",
           CONFIG_PATH, cfg->focus_driver, cfg->focus_enabled,
           cfg->autofocus_enabled, cfg->rotate_180, cfg->mic_enabled,
           cfg->mic_gain);
  return ESP_OK;
}
...
esp_err_t config_load(camera_config_t *out) {
  ...
  ESP_LOGI(TAG, "loaded %s (driver=%s focus=%d af=%d rot180=%d mic=%d gain=%d)",
           CONFIG_PATH, out->focus_driver, out->focus_enabled,
           out->autofocus_enabled, out->rotate_180, out->mic_enabled,
           out->mic_gain);
  return ESP_OK;
}
