...
typedef struct {
  ...
  uint16_t audio_bits_per_sample; // for the WAVEFORMATEX header only; MP3 uses
                                  // 0 here conventionally but 16 is accepted
  uint32_t audio_format_tag;      // WAVEFORMATEX wFormatTag (0x0055 = MP3)
  uint32_t audio_avg_bytes_per_sec; // CBR bitrate/8
  uint16_t audio_samples_per_frame; // MP3 samples per frame (1152 for MPEG-1,
                                    // 576 for MPEG-2)
  ...
  size_t movi_data_start; // offset of the "movi" FourCC (first byte of movi
                          // contents)
  ...
  size_t auds_length_pos; // strh.dwLength (audio stream, sample count in AVI
                          // "samples" = MP3 frames)

  // Running counters.
  uint32_t video_frames_written;
  uint32_t audio_samples_written;
  uint32_t audio_frames_written; // MP3-chunk count — goes into the audio strh
                                 // dwLength
  uint32_t
      video_bytes_written; // raw payload bytes, for max_bytes_per_sec estimate
