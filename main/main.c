...
static cfg_item_t g_cfg_items[] = {
    ...
    {"Mic Gain", CFG_KIND_INT, &g_cfg.mic_gain, CONFIG_MIC_GAIN_MIN,
     CONFIG_MIC_GAIN_MAX},
};
...
static bool cfg_item_enabled(const cfg_item_t *it) {
  if (it->target == &g_cfg.autofocus_enabled)
    return g_focus_present;
  return true;
}
...
static cfg_act_result_t cfg_item_activate(int idx) {
  ...
  if (it->kind == CFG_KIND_DRIVER) {
    ...
    int cur_idx = -1;
    for (int i = 0; i < n; i++) {
      if (focus_driver_by_index(i) == cur) {
        cur_idx = i;
        break;
      }
    }
...
static void blit(void) {
  ...
  esp_lcd_panel_draw_bitmap(s_panel, 0, 0, display_h_res, display_v_res,
                            fb.pixels);
}

static void splash(uint16_t bg, uint16_t fg, const char *line1,
                   const char *line2) {
  fbdraw_clear(&fb, bg);
...
void app_main(void) {
  ...
  if (g_cfg.focus_enabled) {
    if (focus_select(g_cfg.focus_driver) == ESP_OK) {
      g_focus_pos = focus_active()->pos_default;
      g_focus_present = true;
      ESP_LOGI(TAG, "focus driver '%s' ready, pos=%u", focus_active_name(),
               g_focus_pos);
    } else {
...
#define SHOW_BANNER(fmt, ...)                                                  \
  ...
  while (1) {
      ...
      if (event.type == INPUT_EVENT_TYPE_NAVIGATION &&
        ...
        case BSP_INPUT_NAVIGATION_KEY_RETURN:
          if (mode == MODE_CONFIG && CFG_ITEM_COUNT > 0) {
            ...
            if (!microphone_is_running()) {
              ...
              esp_err_t derr = microphone_debug_raw_dump("/sd/miccal.wav", 5);
              if (derr == ESP_OK) {
        ...
        case BSP_INPUT_NAVIGATION_KEY_F1:
          ...
          if (mode != MODE_VIEW) {
            ...
            esp_err_t verr =
                viewer_open(DCIM_PATH, preview_area_w, preview_area_h);
            if (verr == ESP_OK) {
              mode = MODE_VIEW;
        ...
        case BSP_INPUT_NAVIGATION_KEY_F2:
          ...
          if (mode != MODE_PHOTO) {
            ...
            esp_err_t serr = switch_pipeline_to_source(
                &sensor, false, preview_area_w, preview_area_h);
            if (serr != ESP_OK) {
        ...
        case BSP_INPUT_NAVIGATION_KEY_F3:
          ...
          if (mode != MODE_VIDEO) {
            ...
            esp_err_t serr = switch_pipeline_to_source(
                &sensor, true, preview_area_w, preview_area_h);
            if (serr != ESP_OK) {
              SHOW_BANNER("Mode switch failed (%d)", serr);
              break;
            }
        ...
        case BSP_INPUT_NAVIGATION_KEY_VOLUME_DOWN:
          ...
          if (mode == MODE_VIDEO) {
            ...
            if (newg < CONFIG_MIC_GAIN_MIN)
              newg = CONFIG_MIC_GAIN_MIN;
            if (newg > CONFIG_MIC_GAIN_MAX)
              newg = CONFIG_MIC_GAIN_MAX;
    ...
    if (display_ready) {
      ...
      fbdraw_fill_rect(&fb, (int)hud_area_x, 0, (int)hud_area_w, (int)logical_h,
                       BLACK);
      ...
      if (mode == MODE_VIEW && viewer_has_image()) {
        ...
        for (int i = 0; i < CFG_ITEM_COUNT; ++i) {
          ...
          uint16_t col = sel ? (avail ? YELLOW : DIM) : (avail ? WHITE : DIM);
          if (sel) {
            fbdraw_fill_rect(&fb, mx - row_pad, my - row_pad,
                             (int)preview_area_w - 2 * (mx - row_pad), row_h,
                             MENU_HL_BG);
          }
          char row[80];
          if (it->kind == CFG_KIND_DRIVER) {
            ...
            snprintf(row, sizeof(row), "%-9s < %s >", it->label, shown);
          } else if (it->kind == CFG_KIND_INT) {
            int iv = *(int *)it->target;
            snprintf(row, sizeof(row), "%-9s < %d >", it->label, iv);
          } else {
            bool v = *(bool *)it->target;
            snprintf(row, sizeof(row), "[%c] %s%s", v ? 'x' : ' ', it->label,
                     avail ? "" : " (needs Focus)");
          }
      ...
      if (g_focus_dir != 0 && g_focus_present && fd && mode != MODE_VIEW &&
          mode != MODE_CONFIG) {
        ...
        if (step > 0) {
          ...
          if (np < (int32_t)fd->pos_min)
            np = fd->pos_min;
          if (np > (int32_t)fd->pos_max)
            np = fd->pos_max;
      ...
      if (g_focus_present && g_cfg.autofocus_enabled && mode != MODE_VIEW &&
          mode != MODE_CONFIG) {
...
#define KEY_TEXT_DY ((KEY_ROW_H - hud_font) / 2)
    ...
    if (++prof_n >= 15) {
      prof_n = 0;
      /*
      ESP_LOGI(TAG, "loop: wait=%lld bg=%lld img=%lld hud=%lld blit=%lld
      ready=%d (us)", (long long)(t_after_wait  - t_wait_start), (long
      long)(t_after_bg    - t_after_wait), (long long)(t_after_image -
      t_after_bg), (long long)(t_after_hud   - t_after_image), (long
      long)(t_after_blit  - t_after_hud), display_ready);
       */
    }
    ...
    if (space_pending) {
      ...
      if (mode == MODE_PHOTO) {
        ...
        esp_err_t err = photo_capture(&sensor, preview_area_w, preview_area_h,
                                      PREVIEW_TARGET_FPS, DCIM_PATH, saved_path,
                                      sizeof(saved_path));
        ...
        if (video_is_recording()) {
          ...
          char vid_path[128] = {0};
          esp_err_t err =
              video_record_start(DCIM_PATH, vid_path, sizeof(vid_path));
