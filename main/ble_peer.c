...
#if MYNEWT_VAL(BLE_INCL_SVC_DISCOVERY) ||                                      \
    MYNEWT_VAL(BLE_GATT_CACHING_INCLUDE_SERVICES)
static int peer_inc_disced(uint16_t conn_handle,
                           const struct ble_gatt_error *error,
...
#if (MYNEWT_VAL(BLE_INCL_SVC_DISCOVERY) ||                                     \
     MYNEWT_VAL(BLE_GATT_CACHING_INCLUDE_SERVICES))
static struct peer_incl_svc *peer_incl_svc_find_prev(const struct peer_svc *svc,
                                                     uint16_t incl_svc_handle) {
...
static int peer_inc_add(struct peer *peer, uint16_t svc_start_handle,
  ...
  BLE_HS_LOG(DEBUG, "Inc Service added with handle = %d",
             gatt_incl_svc->handle);
...
static void peer_disc_incs(struct peer *peer) {
  ...
  if (svc != NULL && !peer_svc_is_empty(svc)) {
    rc = ble_gattc_find_inc_svcs(peer->conn_handle, svc->svc.start_handle,
                                 svc->svc.end_handle, peer_inc_disced, peer);
    if (rc != 0) {
      peer_disc_chrs(peer);
    }
  } else {
...
void peer_list_all(const struct peer *peer) {
  ...
  printf("Peer handle=%d\r\n", peer->conn_handle);
  SLIST_FOREACH(svc, &peer->svcs, next) {
    ble_uuid_to_str((ble_uuid_t *)&svc->svc.uuid, buffer);
    printf("  Service start_handle=%d end_handle=%d uuid=%s\r\n",
           svc->svc.start_handle, svc->svc.end_handle, buffer);
    SLIST_FOREACH(chr, &svc->chrs, next) {
      ble_uuid_to_str((ble_uuid_t *)&chr->chr.uuid, buffer);
      printf("    Characteristic def_handle=%d val_handle=%d properties=0x%02x "
             "uuid=%s\r\n",
             chr->chr.def_handle, chr->chr.val_handle, chr->chr.properties,
             buffer);
      SLIST_FOREACH(dsc, &chr->dscs, next) {
        ble_uuid_to_str((ble_uuid_t *)&dsc->dsc.uuid, buffer);
        printf("      Descriptor handle=%d uuid=%s\r\n", dsc->dsc.handle,
               buffer);
      }
    }
  }
...
const struct peer_svc *peer_svc_find_uuid(const struct peer *peer,
  ...
  SLIST_FOREACH(svc, &peer->svcs, next) {
    if (ble_uuid_cmp(&svc->svc.uuid.u, uuid) == 0) {
      // printf("peer_svc_find_uuid: service found\r\n");
      return svc;
    }
    ble_uuid_to_str((ble_uuid_t *)&svc->svc.uuid, buffer);
    // printf("peer_svc_find_uuid: has not matching service with UUID %s\r\n",
    // buffer);
  }
...
const struct peer_chr *peer_chr_find_uuid(const struct peer *peer,
  ...
  ble_uuid_to_str(svc_uuid, buffer);
  // printf("peer_chr_find_uuid: searching for service with UUID %s\r\n",
  // buffer);

  svc = peer_svc_find_uuid(peer, svc_uuid);
  if (svc == NULL) {
    // printf("peer_chr_find_uuid: service not found\r\n");
    return NULL;
  }

  ble_uuid_to_str(chr_uuid, buffer);
  // printf("peer_chr_find_uuid: searching for characteristic with UUID %s\r\n",
  // buffer);

  SLIST_FOREACH(chr, &svc->chrs, next) {
    if (ble_uuid_cmp(&chr->chr.uuid.u, chr_uuid) == 0) {
      // printf("peer_chr_find_uuid: characteristic found\r\n");
      return chr;
    }
    ble_uuid_to_str((ble_uuid_t *)&chr->chr.uuid, buffer);
    // printf("peer_chr_find_uuid: has not matching characteristic with UUID
    // %s\r\n", buffer);
  }

  // printf("peer_chr_find_uuid: characteristic not found\r\n");
...
const struct peer_dsc *peer_dsc_find_uuid(const struct peer *peer,
  ...
  // printf("peer_dsc_find_uuid: searching for descriptor with UUID ");

  SLIST_FOREACH(dsc, &chr->dscs, next) {
    if (ble_uuid_cmp(&dsc->dsc.uuid.u, dsc_uuid) == 0) {
      // printf("peer_dsc_find_uuid: descriptor found\r\n");
      return dsc;
    }
    ble_uuid_to_str((ble_uuid_t *)&dsc->dsc.uuid, buffer);
    // printf("peer_dsc_find_uuid: has not matching descriptor with UUID
    // %s\r\n", buffer);
  }

  // printf("peer_dsc_find_uuid: descriptor not found\r\n");
...
#if MYNEWT_VAL(BLE_INCL_SVC_DISCOVERY) ||                                      \
    MYNEWT_VAL(BLE_GATT_CACHING_INCLUDE_SERVICES)
int peer_init(int max_peers, int max_svcs, int max_incl_svcs, int max_chrs,
              int max_dscs)
#else
...
#endif
{
  ...
  if (rc != 0) {
    ...
    MYNEWT_VAL(BLE_GATT_CACHING_INCLUDE_SERVICES)
  peer_incl_svc_mem =
      malloc(OS_MEMPOOL_BYTES(max_incl_svcs, sizeof(struct peer_incl_svc)));
