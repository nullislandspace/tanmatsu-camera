...
static esp_err_t encode_and_write(const uint8_t *rgb565, uint32_t width,
                                  uint32_t height, const char *out_path) {
  ...
  uint32_t jpg_size = 0;
  err = jpeg_encoder_process(enc, &enc_cfg, rgb565,
                             (uint32_t)(width * height * 2u), jpg_buf,
                             (uint32_t)jpg_buf_sz, &jpg_size);
  if (err != ESP_OK) {
