...
static inline int hershey_draw_char(pax_buf_t *buf, pax_col_t color,
                                    float screen_x, float screen_y, char c,
                                    float font_height) {
...
static inline pax_vec2f hershey_draw_string(pax_buf_t *buf, pax_col_t color,
  ...
  while (*str) {
    screen_x +=
        hershey_draw_char(buf, color, screen_x, screen_y, *str, font_height);
    str++;
  }
...
// font_height: desired font height in pixels
static inline pax_vec2f hershey_string_size(float font_height,
                                            const char *str) {
  float scale = font_height / HERSHEY_BASE_HEIGHT;
  int width = 0;
  while (*str) {
