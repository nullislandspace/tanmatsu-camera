...
static esp_err_t scan_dcim(const char *dcim_dir) {
  ...
  while ((de = readdir(dir)) != NULL && s_count < MAX_FILES) {
    ...
    if (nlen < 8)
      continue;
    if (strncmp(de->d_name, "IMG_", 4) != 0)
      continue;
    if (strcmp(de->d_name + nlen - 4, ".jpg") != 0)
      continue;
...
static esp_err_t load_image(int index) {
  ...
  if (err != ESP_OK || info.width == 0 || info.height == 0) {
    ESP_LOGE(TAG, "jpeg_decoder_get_info: %d (%" PRIu32 "x%" PRIu32 ")", err,
  ...
  s_index = index;
  ESP_LOGI(TAG,
           "loaded %s: src %" PRIu32 "x%" PRIu32 " -> disp %" PRIu32 "x%" PRIu32
           " (frag %" PRIu32 "/16)",
           s_files[index], info.width, info.height, s_scaled_w, s_scaled_h,
           frag);
  return ESP_OK;
}

esp_err_t viewer_open(const char *dcim_dir, uint32_t max_w, uint32_t max_h) {
  if (!dcim_dir || max_w == 0 || max_h == 0)
    return ESP_ERR_INVALID_ARG;
  ...
  s_disp_buf_sz =
      (s_disp_buf_sz + cache_line - 1u) & ~((size_t)cache_line - 1u);
...
void viewer_close(void) {
  ...
  if (s_ppa_done) {
    vSemaphoreDelete(s_ppa_done);
    s_ppa_done = NULL;
  }
  if (s_jpeg_dec) {
    jpeg_del_decoder_engine(s_jpeg_dec);
    s_jpeg_dec = NULL;
  }
  if (s_disp_buf) {
    free(s_disp_buf);
...
bool viewer_has_image(void) {
  return s_count > 0 && s_disp_buf != NULL && s_scaled_w > 0;
}
