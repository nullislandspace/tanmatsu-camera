...
esp_err_t sdcard_init(void) {
  ...
  if (res != ESP_OK) {
    ESP_LOGE(TAG, "Failed to create SD LDO power control: %s",
             esp_err_to_name(res));
    return res;
  }

  // Power cycle the SD card to ensure it's in a known state
  // This prevents issues when the card was left in SDMMC mode from a previous
  // session
  ...
  res = esp_vfs_fat_sdmmc_mount(mount_point, &host, &slot_config, &mount_config,
                                &card);
  ...
  if (res != ESP_OK) {
    if (res == ESP_FAIL) {
      ...
      ESP_LOGE(TAG, "HINT: Please verify if there is an SD card inserted into "
                    "the SD slot. Then, try rebooting the board.");
    }
    return res;
  }
