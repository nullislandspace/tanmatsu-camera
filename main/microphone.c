...
static void mic_task_fn(void *arg) {
  ...
  while (s_running) {
    size_t got_bytes = 0;
    esp_err_t err =
        i2s_channel_read(s_rx_handle, raw, MIC_DMA_FRAMES * sizeof(int32_t),
    ...
    if (s_dbg_active && s_dbg_buffer) {
      ...
      if (to_copy > 0) {
        memcpy((int32_t *)s_dbg_buffer + s_dbg_write_ix, raw,
    ...
    for (size_t i = 0; i < n; i += 2) {
      ...
      int64_t acc1 = (int64_t)s32 * (int64_t)MIC_LPF_ALPHA_Q16 +
                     (int64_t)lpf_prev1 * (int64_t)one_minus_alpha;
      int32_t y1 = (int32_t)(acc1 >> 16);
      lpf_prev1 = y1;
      int64_t acc2 = (int64_t)y1 * (int64_t)MIC_LPF_ALPHA_Q16 +
    ...
    if (win_us >= 5000000) {
      uint32_t raw_hz =
          (uint32_t)(total_raw_samples * 1000000ull / (uint64_t)win_us);
      uint32_t out_hz =
          (uint32_t)(total_emitted_samples * 1000000ull / (uint64_t)win_us);
      uint32_t drops = s_dma_drops - drops_at_window_start;
      ESP_LOGI(TAG, "rate: raw=%u Hz emitted=%u Hz (cfg=%u Hz) drops=%u",
               (unsigned)raw_hz, (unsigned)out_hz, (unsigned)MIC_I2S_RATE_NOM,
               (unsigned)drops);
...
esp_err_t microphone_start(void) {
  ...
  ESP_LOGI(
      TAG,
      "started (I2S%d, %u Hz nominal stereo → LEFT mono, SCK=%d WS=%d DIN=%d)",
      (int)MIC_I2S_PORT, (unsigned)MIC_I2S_RATE_NOM, (int)MIC_BCLK_GPIO,
      (int)MIC_WS_GPIO, (int)MIC_DIN_GPIO);
  return ESP_OK;
}
...
bool microphone_is_running(void) { return s_running; }
...
int microphone_get_gain(void) { return s_gain; }

size_t microphone_read_pcm(int16_t *dst, size_t n_samples, TickType_t timeout) {
  if (!s_running || !s_pcm_stream || !dst || n_samples == 0)
    return 0;
  ...
  while (got_bytes < target_bytes) {
    ...
    size_t n = xStreamBufferReceive(s_pcm_stream, (uint8_t *)dst + got_bytes,
                                    target_bytes - got_bytes, remaining);
    if (n == 0)
      break; // timeout reached with nothing new
    got_bytes += n;
  }
...
esp_err_t microphone_debug_raw_dump(const char *path, int seconds) {
  ...
  if (!buf) {
    ESP_LOGE(TAG, "dump: PSRAM alloc %u bytes failed",
  ...
  if (!f) {
    ...
    if (w != written_samples) {
      ESP_LOGE(TAG, "dump: short write %u/%u", (unsigned)w,
