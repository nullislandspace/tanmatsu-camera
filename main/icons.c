...
static const char *icon_filenames[ICON_KEY_COUNT] = {
    [ICON_ESC] = "/int/icons/esc.png", [ICON_F1] = "/int/icons/f1.png",
    [ICON_F2] = "/int/icons/f2.png",   [ICON_F3] = "/int/icons/f3.png",
    [ICON_F4] = "/int/icons/f4.png",   [ICON_F5] = "/int/icons/f5.png",
...
static FILE *open_icon(int i, const char **out_path) {
  if (i == ICON_ESC) {
    ...
    if (f) {
      *out_path = APP_ESC_PATH_SD;
      return f;
    }
    f = fopen(APP_ESC_PATH_INT, "rb");
    if (f) {
      *out_path = APP_ESC_PATH_INT;
      return f;
    }
  }
...
static inline uint16_t blend_to_565(uint16_t dst565, uint8_t sr, uint8_t sg,
                                    uint8_t sb, uint8_t a) {
  if (a == 0xFF)
    return ((sr & 0xF8u) << 8) | ((sg & 0xFCu) << 3) | (sb >> 3);
