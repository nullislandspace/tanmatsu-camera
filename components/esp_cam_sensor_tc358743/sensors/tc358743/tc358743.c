...
static esp_err_t tc358743_power_on(esp_cam_sensor_device_t *dev) {
  ...
  /* LOCAL DEVIATION from the upstream component (tanmatsu-camera):
   * upstream waits 500 ms here for the internal power-on reset when no
   * reset pin is wired -- which, on this board, is always, because the
   * Tanmatsu camera connector has no reset line and camera_sensor.c
   * passes reset_pin = -1.
   *
   * power_on() runs on the *probe* path. esp_cam_sensor's detect loop
   * walks every registered driver in link order, so with the delay
   * here every board that is not a TC358743 pays half a second at boot
   * before the I2C read at 0x0F fails. That is a real regression on
   * the only hardware anyone can currently test.
   *
   * The wait is moved to set_format(), which is the first thing that
   * actually configures the chip and the only place the POR state
   * matters. Reading CHIPID needs nothing but a powered I2C target,
   * and the module's 3V3 rail is up from board power-on -- so by the
   * time anything probes, any POR finished long ago. */
...
static esp_err_t tc358743_set_format(esp_cam_sensor_device_t *dev,
  ...
  /* Internal power-on reset settling time, moved here from
   * power_on() so the probe path stays cheap -- see the comment
   * there. Only paid on a board that really has a TC358743. */
  if (dev->reset_pin < 0) {
    delay_ms(500);
  }
